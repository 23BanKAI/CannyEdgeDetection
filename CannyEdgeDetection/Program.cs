...
class CannyEdgeDetection
{
    static void Main()
    {
        ...
        Bitmap grayscaleImage = ConvertToGrayscale(inputImage);

        // Применение фильтра Гаусса для сглаживания
        Bitmap blurredImage = ApplyGaussianBlur(grayscaleImage);

        // Вычисление градиентов по оси X и Y
        (Bitmap gradientMagnitude, Bitmap gradientDirection) = ComputeGradients(blurredImage);

        // Применение не-максимумного подавления
        Bitmap suppressedImage = ApplyNonMaximumSuppression(gradientMagnitude, gradientDirection);

        // Применение гистерезиса для порогового отсечения
        Bitmap edgesImage = ApplyHysteresisThresholding(suppressedImage, lowThreshold: 30, highThreshold: 80);

        // Сохранение результирующего изображения
        edgesImage.Save("output_image.jpg");

        Console.WriteLine("Обнаружение границ методом Кэнни завершено. Результирующее изображение сохранено в output_image.jpg");
    }
    ...
    static Bitmap ApplyGaussianBlur(Bitmap inputImage)
    {
    ...
    static (Bitmap gradientMagnitude, Bitmap gradientDirection) ComputeGradients(Bitmap inputImage)
    {
    ...
    static Bitmap ApplyNonMaximumSuppression(Bitmap gradientMagnitude, Bitmap gradientDirection)
    {
    ...
    static Bitmap ApplyHysteresisThresholding(Bitmap suppressedImage, int lowThreshold, int highThreshold)
    {
